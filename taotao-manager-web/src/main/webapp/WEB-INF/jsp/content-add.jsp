...
<script type="text/javascript">
	...
	$(function(){
		...
		TT.initOnePicUpload();  //初始化单张图片上传器
		$("#contentAddForm [name=categoryId]").val($("#contentCategoryTree").tree("getSelected").id);//取出id
	});
	...
	var contentAddPage  = {
			submitForm : function (){
				...
				contentAddEditor.sync();
															//表单序列化,将表单内容变成key-value的形式
				$.post("/content/save",$("#contentAddForm").serialize(), function(data){
					if(data.status == 200){//当结果是200 则返回数据是taotaoResult
