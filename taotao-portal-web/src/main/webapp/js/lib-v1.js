...
var category = {OBJ: $("#_JD_ALLSORT"),
		...
		URL_Serv: "http://localhost:8092/rest/itemcat/list?callback=category.getDataService",
...
//源代码        return "" == c ? d : (/^http[s]?:\/\/([\w-]+\.)+[\w-]+([\w-.\/?%&=]*)?$/.test(c) || (c = c.replace(/-000$/, ""), c = c.match(/^\d*-\d*$/) ? "http://channel.jd.com/" + c + ".html" : "http://www.taotao.com" + c + ".html"), '<a href="' + c + '">' + d + "</a>")
        		return "" == c ? d : (/^http[s]?:\/\/([\w-]+\.)+[\w-]+([\w-.\/?%&=]*)?$/.test(c) || (c = c.replace(/-000$/, ""), c = c.match(/^\d*-\d*$/) ? "http://channel.jd.com/" + c + ".html" : "http://localhost:8085/search.html?" + c), '<a href="' + c + '">' + d + "</a>")
       ...
       /* var a, b = {}, c = $("#categorys-2013");
    	...
    	//使用jsonp来实现跨域请求    ajax内置的方法		(请求url,回调方法)
